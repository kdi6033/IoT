...
int type=29; // 기기 인식번호 -> display에 사용 6= LS PLC XEC-DR14E
...
const char* clientName = ""; //setup에서 생성
...
void tick()
{
  ...
  //if((countTick%5)==0)
    tickMeasure();
  // 접속하면 서버에 자동 등록하기 위해 5회 통신하고 그 다음 부터는 값이 변할 때만 전송한다.
...
void crd16Rtu2() {
  ...
  char str2[24];
  //출력
  str2[0]=0xff; str2[1]=0x05; str2[2]=0x00; str2[3]=0x03; str2[4]=0x00;
  str2[5]=0x00; str2[6]=0x00; str2[7]=0x00; 
  len2=6;

  inputString2 = "";
  uint8_t * data2 = (uint8_t *) &str2[0];
  si2=crc16(data2, len2, 0x8005, 0xFFFF, 0x0000, true,  true  );
  sj2=si2&0xff;
  str2[len2]=sj2;
  sj2=si2>>8;
  str2[len2+1]=sj2;
...
/*void callback(char* topic, byte* payload, unsigned int length) {
  ...
  for (int i = 0; i < length; i++) {
   ...
   /* if(int(payload[length] - '1'))
    {
      crd16Rtu();
    }
    else if(int(payload[length -3] -'0'))
    {
      crd16Rtu2();
    }*/
 /* }
