...
int type=10; // 기기 인식번호 -> display에 사용
...
int timeMqtt=5;
float temp=0.,humi=0.,ec=0.;
int sound=0,soundPre=1,soundValue;
...
void upWebSocket() {
  //HTML로 보냄
  String json = "{\"sound\":"; json += (String)analogRead(S);
  json += "}";
...
void serialEvent() {
  ...
  if(inputString.length() >= 11) {
    ...
    ss=((int)inputString.charAt(7)*255+(int)inputString.charAt(8));
    ec=ss.toFloat();
    Serial.println("EC : "+ss+" uS/cm");
    inputString="";
    Serial.println("");
  }
}
