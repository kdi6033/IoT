  ...
  <table>
    <tr>
      <th><span class="dht-labels">온도 : </span> </th>
        <th><span id="temp">%TEMPERATURE%</span>
          <sup class="units">&deg;C</sup></th>
    </tr>
    <tr>
      <th><label>습도 : </label></th>
      <th><div id="humi">%humidity%</div></th>
    </tr>
  </table>
...
void handleWifi() {
  ...
  s+="<table>";
    ...
    s+="<tr>";
      s+="<th>E-mail</th>";
      s+="<th><input type='test' value='"+(String)email+"' name='e'/></th>";
    s+="</tr>";
...
void handleWifiSave() {
  ...
  server.arg("e").toCharArray(email, sizeof(email) - 1);
...
void handleScan() {
  String s;
  s="{\"mac\":\""+sMac+"\",\"ip\":\""+WiFi.localIP().toString()+"\",\"type\":"+type+",\"temp\":"+temp+",\"humi\":"+humi+"}";
  server.send(200, "text/html", s);
}
