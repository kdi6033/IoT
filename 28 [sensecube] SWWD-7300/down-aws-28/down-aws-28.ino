...
int type=28; // 기기 인식번호 -> display에 사용
...
void tickMeasure() {
  ...
  //OLED로 표시
  String s;
   s="Wspeed  "+(String)wind_direction+"  ";
...
//    Serial.println(c);         // print the character
  ...
  wind_direction = ((int)c+(int)x*256);
}
...
void upWebSocket() {
  //HTML로 보냄
  String json = "{\"wind_direction\":"; json += wind_direction;
  json += "}";
...
void tickMqtt() {
  ...
  json += ",\"wind_direction\":"; json += wind_direction;//String(wind_direction);
  json += "}";
