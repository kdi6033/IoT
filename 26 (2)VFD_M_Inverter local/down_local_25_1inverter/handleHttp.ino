...
char Body[] PROGMEM = R"=====(
    <h4> VFD-M 인버터 </h4>
      <th><span id="id1"><input type='radio' act='2' id='+data.mac+' value='0' onclick='handleRadio(event)'/>정지<input type='radio' act='2' id='"+data.mac+"' value='1' onclick='handleRadio(event)'/>정회전<input type='radio' act='2' id='"+data.mac+"' value='2' onclick='handleRadio(event)'/>역회전</span></th>
    <br>
      <th>&emsp;<span id="id2">속도<input type='range' act='3' id='+data.mac+' min='0' max='6000' value='2000' step='10' onchange='handleRange(event)'></span></th>
    <script>
        function handleRange(e){ 
            Socket.send("{'vel':"+e.target.value+"}");
        }
        function handleRadio(e){  
            Socket.send("{'dir':"+e.target.value+"}");
        }
    </script>
)=====";
...
void handleScan() {
  String s;
  s="{\"mac\":\""+sMac+"\",\"ip\":\""+WiFi.localIP().toString()+"\",\"type\":"+type+",\"dir\":\""+dir+"\",\"vel\":\""+vel+"\"}";
  server.send(200, "text/html", s);
}
