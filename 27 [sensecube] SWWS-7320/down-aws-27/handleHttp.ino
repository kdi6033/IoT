...
char ScriptRoot[] PROGMEM = R"=====(
  <script>
    var Socket;
    function init() {
      Socket = new WebSocket('ws://' + windspeedow.location.hostname + ':81/');
...
char Body[] PROGMEM = R"=====(
  ...
  <table>
    <tr>
      <th><label>Windspeed : </label></th>
      <th><span id="windspeed">%Windspeed%</span></th>
    </tr>
  </table>
...
void handleScan() {
  String s;
  s="{\"mac\":\""+sMac+"\",\"ip\":\""+WiFi.localIP().toString()+"\",\"type\":"+type+",\"windspeed\":"+windspeed+"}";
  server.send(200, "text/html", s);
}
